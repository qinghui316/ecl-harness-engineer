...
Line 56-75:  Context loading: AGENTS.md, docs/ECL.md, active change if present, otherwise auto-evolve pending if present, otherwise STATUS, then task-specific project docs
...
- Project identity and ECL constraints must not compete: keep the first screen project-first,
  ...
  `docs/ECL.md`, active change files when present, otherwise `harness/evolution/pending.md`
  when it exists, otherwise `docs/STATUS.md`, then README/architecture/design/reference docs.
...
- State that `harness/evolution/pending.md`, when present and no active change exists, should be
  handled before ordinary STATUS resume work. This pending file is generated by scripts and should
  not cause full-archive loading.
- Historical archive loading must be selective: start from `docs/STATUS.md` paths or
  `harness/changes/INDEX.json`, read archived `summary.md` first, and read spec/tasks/reviews
...
- Stage-boundary update protocol for `summary.md`, `spec.md`, `tasks.md`, and `reviews/`
...
- Auto-evolve handling: `harness-change close/reindex` may generate `harness/evolution/pending.md`; Codex applies evidence-backed harness deltas only when no active change is present
...
- If `harness/evolution/pending.md` exists after close and no active task is present, treat auto-evolve as the next recommended work unless a user task is urgent.
