...
- Every started pending evolution includes an Experience Retention Scan. Classify repeated lessons
  and current-doc content as Promote, Retain, Merge, Retire, or Archive-only before deciding keep,
  rejected, noop, or revert.
- Auto-evolve is not append-only documentation growth. Merge duplicate current facts, retire stale
  current-state language, and keep historical narrative in archive summaries unless it is still a
  current reusable constraint.
...
|-----------|----------|----------|--------------------|
| [lesson/rule] | Promote / Retain / Merge / Retire / Archive-only | `harness/changes/archive/.../summary.md` | [edit target or "none"] |

- Promote only repeated, current, project-relevant constraints.
- Merge duplicated current facts into one authoritative current doc.
- Retire stale "current plan", "baseline", "next phase", or roadmap language when newer evidence supersedes it.
- Keep detailed historical narrative in archive summaries; link to it selectively instead of copying it forward.
...
- AGENTS.md role/size checked:
- STATUS.md remains handoff, not archive ledger:
- Duplicate closeout/current-state facts checked:
- Stale roadmap/current-plan language checked:
...
11. Documentation entropy control for AGENTS, STATUS, current docs, and archive links
12. Experience lifecycle decisions: Promote, Retain, Merge, Retire, Archive-only
13. Script command reference
14. Rule: `INDEX.json` is generated by script only

Keep philosophy brief. This is an operating manual.

### 4.1 Documentation Entropy Control

Include a short section in generated `docs/ECL.md` with these rules:

- `AGENTS.md` is a project navigation map. New/simple harnesses usually target about 80-120 lines;
  mature harnesses may reach about 120-180 lines only when extra lines carry current project
  navigation, current operating constraints, or verification guidance.
- `docs/STATUS.md` is a current handoff map, not a full changelog, phase ledger, or archive
  database.
- Current docs must not copy complete closeout narratives from archived changes. Link to selected
  archive summaries or query `harness/changes/INDEX.json` instead.
- Stale "current plan", "next phase", "baseline", or roadmap language must be retired,
  historicalized, or moved to archive-only references when newer active/STATUS/archive evidence
  supersedes it.
- Line counts are alarms, not the only metric. A short but vague AGENTS is still bad; a mature but
  justified AGENTS can be acceptable when it remains navigable.

### 4.2 Experience Lifecycle

Include a short section in generated `docs/ECL.md` with these decisions for auto-evolve and manual
documentation cleanup:

| Decision | Use When |
|----------|----------|
| Promote | Repeated archive evidence proves a current reusable rule, template, lint, or verification gate |
| Retain | The current text is still accurate, concise, and useful |
| Merge | Multiple current docs repeat the same fact or closeout narrative |
| Retire | Text describes obsolete current state, superseded plans, or old baselines |
| Archive-only | Detail is historically useful but not needed in current docs |

A `noop` auto-evolve result is valid only after this scan explains why no durable current-doc or
mechanical-rule change should be applied.
...
5. Run the Experience Retention Scan: classify candidates as Promote, Retain, Merge, Retire, or
   Archive-only, including a reason when the final decision is noop.
6. Generate `harness/evolution/proposals/YYYY-MM-DD-auto-evolve.md` from the proposal template in docs/ECL.md or references/ecl-harness.md before editing harness files.
7. Request one independent auditor/subagent score before applying. User approval to handle pending
...
8. Apply only accepted candidates with archive evidence, project relevance, score >= 80, and independent approval.
9. Prefer clarifying existing rules over adding new sections, documents, scripts, or workflows.
10. Run harness checks and relevant business gates.
11. Record one terminal result in `harness/evolution/results.tsv`: `keep` for accepted deltas,
...
12. Run `harness-evolve mark-complete` after writing the result. If you cannot complete these
