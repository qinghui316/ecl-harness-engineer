...
## Canonical Knowledge Synchronization

Do not publish unlanded product behavior as shared `current/implemented` knowledge. Record its
knowledge impact in the Change, or use `target/in_progress` for an accepted long-term direction.
After Integration lands canonical code, the same task updates only affected current Markdown and
runs `change reindex`. For non-Git or direct canonical work, do this before Change close. This
lightweight step does not create another Change, run migrate/E1/Judge/renderer/full audit, increment
revision, or run unrelated project tests. If it fails, report `refresh-needed`; canonical code
remains the source of truth and is not rolled back.
...
4. Start from the five Changes and related knowledge/rule owners. Use an explicit creation delta;
   widen to complete Catalog, rule source, or project analysis only when evidence justifies it.
...
There is no E2. Passing work applies after E1. When no independent reviewer is available, record a
staged candidate as `rejected`; use `noop` only when no candidate exists. Rejected/noop preserve
current content; the recoverable transaction cannot replace mutable state with a stale candidate
copy.
