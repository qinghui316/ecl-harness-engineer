...
| Close | Completion commit and Registry summary | Evidence complete; status is completed/blocked/abandoned |
...
- `reviews/review.md`: plan, code, validation, contract, integration, and documentation review.

Keep complete Change files in project Harness `state/changes/`; publish compact machine-readable facts
to the Registry and bind Git Changes to an exact implementation completion commit.

For Git Changes, close is two-stage: validate shared evidence, commit the implementation, then
record the exact clean HEAD and archive Skill evidence. For non-Git Changes, close remains one-stage.
