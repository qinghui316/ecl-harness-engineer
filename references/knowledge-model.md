...
`agent`; documents generated by full project analysis declare `managed_by: renderer`. An Agent may
take over maintenance of a generated page by changing that field in a normal Structured Change.
A full refresh replaces only generated current-fact documents; exact duplicate IDs or paths fail
closed.
...
- Integration lands accepted code and optional project documentation and records the integration
  base, coordination Registry, and Evolution signals;
...
- A `current` product fact must match code already integrated into the recorded target branch.
  ...
  landing, update it through an ordinary Structured Change, focused Evolution, or full refresh as
  appropriate. Current Harness-process facts do not depend on product-code Integration.
- After five qualified Changes and E1, focused Evolution may update, merge, retire, or promote any
  agent-maintained L1/L2/L3 document. Before Create, search catalog metadata and related content, then
  choose Create, Merge, Replace, Retire, or Archive-only. A full-refresh Evolution refreshes
  current-fact documents generated by full project analysis only when whole-project analysis is
  actually required.
...
- The five-Change trigger is a review cadence, not a four-Change freshness guarantee. Review may be
  ...
  than silently trusting stale Wiki text. Full source scans belong to knowledge check, audit, or a
  full-refresh Evolution. Ordinary document reindex and focused Evolution refresh fingerprints only
