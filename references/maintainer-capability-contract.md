...
| `FWD-MECHANICAL` | Only accepted, evidenced project invariants become executable checks; knowledge inspection is read-only, uses one source-fingerprint contract, and separates structural errors from entropy warnings. | `test_evidenced_mechanical_check_has_positive_negative_and_actionable_failure`, `test_knowledge_scan_and_check_are_read_only`, `test_knowledge_fingerprints_have_one_generation_and_scan_contract`, `test_read_only_knowledge_waits_for_content_publication`, `test_knowledge_check_separates_structural_errors_from_entropy_warnings` |
...
| `FWD-UPGRADE` | A single-Lane Harness preserves identity through Git adoption, rejects marker/manifest mismatch, and repairs current and future worktree links. | `test_single_lane_git_transition_preserves_identity_and_repairs_worktrees`, `test_project_marker_rejects_mismatched_manifest_identity` |
